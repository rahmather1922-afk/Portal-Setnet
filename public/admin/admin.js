...
// Nama perusahaan pengirim invoice — otomatis tampil di kop cetak invoice, tidak perlu diisi manual tiap buat invoice
const NAMA_PERUSAHAAN_INVOICE = "SATNET";
...
const hitungTotalInvoice = (items, pinalty, lessDeposit, dendaSetelahPpn, pungutanPpn) => {
  ...
  const dppNilaiLain = subtotalSetelahPinalty * (11 / 12);
  const ppn = dppNilaiLain * 0.12;
  const totalIncludePpn = subtotalSetelahPinalty + ppn;
  const balanceDue = totalIncludePpn - (Number(lessDeposit) || 0) - (Number(pungutanPpn) || 0) + (Number(dendaSetelahPpn) || 0);
  return { hargaJual, subtotalSetelahPinalty, dppNilaiLain, ppn, totalIncludePpn, balanceDue };
};
...
const InvoicePrintView = ({ invoice }) => {
  ...
  const rows = [
    ...
    ["PPN", totals.ppn],
    ["Total Include Ppn", totals.totalIncludePpn],
  ...
  return (
    <div id="invoice-print-area" style={{ fontFamily: "Arial, sans-serif", color: "#111", padding: "28px", fontSize: "12px" }}>
      <p style={{ fontWeight: 700, fontSize: "16px", letterSpacing: "1px", marginBottom: "2px" }}>{NAMA_PERUSAHAAN_INVOICE}</p>
      <h1 style={{ textAlign: "center", fontSize: "26px", fontWeight: 700, letterSpacing: "1px", marginBottom: "22px", marginTop: "-18px" }}>INVOICE</h1>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", gap: "24px", marginTop: "48px" }}>
          ...
          {invoice.ttd_nama && (
            <div style={{ flex: 1, textAlign: "center" }}>
              <p>Hormat kami,</p>
              <p style={{ fontWeight: 700, marginBottom: "56px" }}>{invoice.ttd_jabatan || "Direktur"}</p>
                          ...
                          pagedLog.map((g, idx) => {
                            ...
                            return (
                              <tr key={g.key} className="hover:bg-gray-50/60 transition">
                                ...
                                <td className="p-3.5 text-center" style={{ color: "var(--ink-soft)" }} title="Fitur jarak lokasi belum tersedia">—</td>
                                <td className="p-3.5 text-center">
                                  {g.masuk ? (
                    ...
                    <div className="xl:col-span-2 bg-white p-5 rounded-2xl border h-fit xl:sticky xl:top-6" style={{ borderColor: "var(--border)" }}>
                      ...
                      <form onSubmit={handleSubmitInvoice} className="space-y-3.5 text-xs">
                        ...
                        <div className="p-3 rounded-xl space-y-1 text-[11px]" style={{ background: "var(--brand-soft)" }}>
                          ...
                          <div className="flex justify-between"><span style={{ color: "var(--ink-soft)" }}>PPN (12%)</span><b style={{ color: "var(--ink)" }}>{fmtRupiah(invTotals.ppn)}</b></div>
